...
export function shelfSlots(node: ShelfNode): SlotDeclaration[] {
  const slots: SlotDeclaration[] = [
    { slotId: 'shelves', label: '搁板', default: SHELF_SLOT_DEFAULT_COLOR },
  ]
  const hasFrame = !(node.style === 'wall-shelf' && node.bracketStyle === 'hidden')
  if (hasFrame) slots.push({ slotId: 'frame', label: '框架', default: SHELF_SLOT_DEFAULT_COLOR })
  const hasBack = node.style === 'cubby' || (node.style === 'bookshelf' && node.withBack)
  if (hasBack) slots.push({ slotId: 'back', label: '后侧', default: SHELF_SLOT_DEFAULT_COLOR })
  return slots
}
