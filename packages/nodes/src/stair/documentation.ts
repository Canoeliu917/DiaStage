...
function buildStraightDocumentation(
  ...
  if (arrowStart && arrowNext) {
    ...
    const labelPoint = arrowDirection
      ...
      : arrowStart
    geometries.push(
      annotationText(labelPoint, direction === 'up' ? '上行' : '下行', DIRECTION_FONT_SIZE, stroke),
    )
  }
  ...
  for (const segmentEntry of entry.segments) {
    ...
    geometries.push(
      annotationText(
        rightAnchor,
        `${riserCount} 级 · 踢面 ${formatConstructionLength(riserHeight, unit, profile, { metricNotation })} · 踏面 ${formatConstructionLength(treadDepth, unit, profile, { metricNotation })} · 净宽 ${formatConstructionLength(segment.width, unit, profile, { metricNotation })}`,
    ...
    if (!railNotePlaced && stair.railingMode !== 'none') {
      ...
      geometries.push(
        annotationText(
          leftAnchor,
          `${{ left: '左侧', right: '右侧', both: '两侧' }[stair.railingMode]}栏杆 · 高 ${formatConstructionLength(stair.railingHeight, unit, profile, { metricNotation })}`,
          ANNOTATION_FONT_SIZE,
          stroke,
        ),
      )
...
function buildCurvedDocumentation(
  ...
  const geometries: FloorplanGeometry[] = [
    annotationText(
      notePoint,
      `${stepCount} 级 · 踢面 ${formatConstructionLength(riserHeight, unit, profile, { metricNotation })} · 踏面（行走线）${formatConstructionLength(treadDepth, unit, profile, { metricNotation })} · 净宽 ${formatConstructionLength(stair.width, unit, profile, { metricNotation })}`,
      ANNOTATION_FONT_SIZE,
      stroke,
    ),
    annotationText(
