...
const doorTypeOptions = [
  { label: '平开', value: 'hinged', available: true },
  { label: '双扇', value: 'double', available: true },
  { label: '双开', value: 'french', available: true },
  { label: '折叠', value: 'folding', available: true },
  { label: '入墙推拉', value: 'pocket', available: true },
  { label: '谷仓门', value: 'barn', available: true },
  { label: '推拉', value: 'sliding', available: true },
] satisfies {
  label: string
...
const garageDoorTypeOptions = [
  { label: '分节提升', value: 'garage-sectional', available: true },
  { label: '卷帘', value: 'garage-rollup', available: true },
  { label: '翻板', value: 'garage-tiltup', available: true },
] satisfies {
  label: string
  value: DoorNode['doorType']
  available: boolean
    ...
    >
      ...
      <PanelSection title="位置">
        <SliderControl
          label={
            <>
              X<sub className="ml-[1px] text-[11px] opacity-70">墙体</sub>
            </>
          }
          max={10}
          min={-10}
      ...
      {showSlideSection && (
        <PanelSection title="滑动">
          <div className="flex flex-col gap-2 px-1 pb-1">
            <div className="space-y-1">
              <span className="font-medium text-[10px] text-muted-foreground/80 uppercase tracking-wider">
                {doorType === 'pocket' ? '入墙推拉' : doorType === 'barn' ? '导轨' : '面板'}
              </span>
              <SegmentedControl
