...
export function CeilingPanel() {
  ...
  const heightPresets =
    unit === 'imperial'
      ? [
          { label: '低（8 英尺）', height: 2.4384 },
          { label: '标准（8 英尺 6 英寸）', height: 2.5908 },
          { label: '高（9 英尺）', height: 2.7432 },
        ]
      : [
          { label: '低（2.4 米）', height: 2.4 },
          { label: '标准（2.5 米）', height: 2.5 },
          { label: '高（3.0 米）', height: 3.0 },
        ]
      ...
      <PanelSection title="高度">
        ...
        {isFollows ? (
          <div className="px-1 text-[11px] text-muted-foreground">
            当前 {formatLinearMeasurement(resolvedHeight, unit, metricNotation)}
          </div>
        ) : (
          <SliderControl
            label="高度"
            ...
            return (
              <ActionButton
                ...
                title={
                  fits
                    ? undefined
                    : `超出当前楼层可用高度（${formatLinearMeasurement(maxHeight, unit, metricNotation)}），请先提高楼层高度。`
                }
              />
            )
          })}
        ...
        {Number.isFinite(maxHeight) && (
          <div className="px-1 pb-1 text-[11px] text-muted-foreground">
            楼层高度限制为 {formatLinearMeasurement(maxHeight, unit, metricNotation)}{' '}
            ；如需更高天花板，请提高楼层高度。
          </div>
        )}
      </PanelSection>
        ...
        {node.holes && node.holes.length > 0 ? (
          <div className="flex flex-col gap-1 pb-2">
            {node.holes.map((hole, index) => {
              ...
              const autoLabel = source === 'elevator' ? '电梯自动开孔' : '楼梯自动开孔'
              return (
                <div
          ...
          <div className="px-2 py-3 text-center text-muted-foreground text-xs">暂无孔洞</div>
        )}
