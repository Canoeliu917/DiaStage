...
describe('block toolbar state', () => {
  ...
  test('formats compact singular and plural selection labels', () => {
    expect(formatBlockSelectionStatus('face', 1)).toBe('1 个面')
    expect(formatBlockSelectionStatus('edge', 2)).toBe('2 个边')
    expect(formatBlockSelectionStatus('vertex', 3)).toBe('3 个顶点')
  })
  ...
  test('shows live bevel width and segment count', () => {
    ...
    ).toBe('倒角 · 宽度 0.2 米 · 6 段 · 拖动调整宽度，滚轮调整分段数')
  })
