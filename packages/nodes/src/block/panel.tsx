...
export default function BlockPanel() {
  ...
  const addMaterialSlot = () => {
    ...
    setSlotNotice({
      nodeId: node.id,
      text: `${result.slotNames[result.slotId] ?? result.slotId} 已应用于 ${selectedFaceIds.length} 个面，使用强调材质。按 P 涂刷可替换。`,
    })
    ...
    : selection.mode !== 'face'
      ...
      : materialSelection.kind === 'empty'
        ...
        : materialSelection.kind === 'mixed'
          ? `${selectedFaceIds.length} 个面 · 混合材质槽`
          : `${selectedFaceIds.length} 个面 · ${slotDeclarations.find((slot) => slot.slotId === materialSelection.slotId)?.label ?? materialSelection.slotId}`

  return (
    <PanelWrapper icon="/icons/cube.webp" onClose={close} title={node.name || '网格体'} width={340}>
      ...
      <PanelSection title="材质槽">
        ...
        <div className="mt-2 flex justify-end">
          <ActionButton
            ...
            title={
              slotEditTitle ?? (selectedFaceIds.length === 0 ? '请先选择一个或多个面' : undefined)
            }
          />
        </div>
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-label={`将 ${slot.label} 应用到选中面`}
                  aria-pressed={active}
                  disabled={!canOperateOnFaces || selectedFaceIds.length === 0 || readOnly}
                ...
                <span className="pointer-events-none relative z-10 flex min-w-0 flex-1 flex-col justify-center py-1.5">
                  ...
                  <span className="truncate px-1 text-[10px] text-muted-foreground">
                    {materialLabel} · {faceCount} {faceCount === 1 ? '面' : '面'}
                  </span>
                </span>
                ...
                {slot.slotId === BLOCK_BODY_SLOT_ID ? (
                  ...
                  <button
                    ...
                    aria-label={`删除 ${slot.label} 材质槽`}
                    disabled={!editing || readOnly}
                    onClick={() => removeMaterialSlot(slot.slotId)}
                    title={slotEditTitle ?? '删除此材质槽，并将其面改用主体材质'}
                    type="button"
                  >
      ...
      <PanelSection title="操作">
        <ActionGroup>
          <ActionButton icon={<Move className="h-4 w-4" />} label="移动" onClick={move} />
          <ActionButton
