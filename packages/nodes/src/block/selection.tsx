  ...
  switch (command.type) {
      ...
      return (
        <div className="space-y-1">
          {(['X', 'Y', 'Z'] as const).map((axis, index) =>
            input(`${axis} 距离`, command.delta[index]!, (value) => ({
              ...command,
    ...
    case 'scale-components':
      return (
        <div className="space-y-1">
          {(['X', 'Y', 'Z'] as const).map((axis, index) =>
            input(`${axis} 缩放`, command.factors[index]!, (value) => ({
              ...command,
    ...
    case 'extrude-faces':
      return input('距离', command.distance, (distance) => ({ ...command, distance }))
    case 'inset-faces':
      return input('量值', command.amount, (amount) => ({ ...command, amount }), {
        min: 0,
        max: 0.95,
      })
    case 'bevel-edges':
      return (
        <div className="space-y-1">
          {input('宽度', command.width, (width) => ({ ...command, width }), { min: 0 })}
          {input(
            '分段数',
            command.segments,
    ...
    case 'loop-cut':
      return (
        <div className="space-y-1">
          {input('位置', command.factor, (factor) => ({ ...command, factor }), {
            min: 0.02,
            max: 0.98,
          })}
          {input(
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-[10px] text-muted-foreground">调整上一步操作 · F9</div>
        </div>
        <button
          aria-label="关闭上一步操作面板"
...
function BlockEditor({
  ...
  useEffect(() => {
    ...
    setError(segments ? null : '环切需要由连续四边形面构成的环')
    ...
    (edgeId: string, event: ThreeEvent<PointerEvent>) => {
      ...
      const updatePreview = (factor: number) => {
        ...
        if (!result.ok || !segments) {
          setError(result.ok ? '无法预览环切' : result.error)
          return false
        }
            ...
            <div className={cn(FLOATING_PANEL_CLASS, 'relative')}>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span>{loopCutActive ? '环切' : bevelActive ? '倒角' : '操作'}</span>
                  <ChevronDown className="h-3.5 w-3.5" />
                </button>
                {toolbarPanel === 'operations' ? (
                  <ToolbarPanelFrame label="网格操作" className="w-80 p-1.5">
              ...
              {lastOperation ? (
                <ToolbarButton
                  ...
                  label={`调整 ${lastOperation.label}（F9）`}
              ...
              <ToolbarButton label="结束编辑模式（Tab）" onClick={exitEditMode} sound={false}>
              ...
              <div className="relative">
                ...
                {toolbarPanel === 'selection' ? (
                  ...
                  >
                    <div className="space-y-1">
                      <ToolbarMenuItem label="全选" onClick={selectAll} shortcut="A" sound={false}>
                        <CircleDot className="h-4 w-4" />
                      </ToolbarMenuItem>
                      <ToolbarMenuItem
                        label="反选"
