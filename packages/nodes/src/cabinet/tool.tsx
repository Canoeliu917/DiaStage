...
const CabinetPlacementTool = ({ defaults }: { defaults?: Record<string, unknown> }) => {
  const template = useMemo(() => resolveCabinetPlacementTemplate(defaults), [defaults])
  ...
  const islandMode = template.snapshot
    ? template.run.countertopBackOverhang > 0
    : placementType === 'island'
  const yawRef = useRef(0)
  const islandModeRef = useRef(islandMode)
  ...
  const previewNodeTemplate = useMemo(() => {
    const runDefaults = template.run
    return CabinetModuleNode.parse({
      ...cabinetModuleDefinition.defaults(),
      ...template.modules[0],
      ...(template.snapshot && { width: runDefaults.width, depth: runDefaults.depth }),
  ...
  const placementDimensions = useMemo(() => {
    ...
    return [
      ...
      previewNode.depth +
        (template.snapshot
          ? defaults.countertopBackOverhang
          : islandMode
            ? ISLAND_SEATING_OVERHANG
            : 0),
    ] as [number, number, number]
  }, [previewNode, islandMode, template])
  ...
  const placementSnapFootprint = useMemo(() => {
    ...
    const backOverhang = template.snapshot
      ? template.run.countertopBackOverhang
      : islandMode
        ? ISLAND_SEATING_OVERHANG
        : 0
    return {
  ...
  }, [islandMode, placementDimensions, previewNode, template])
  ...
  const ghost = useMemo(() => {
    const nodes = Object.fromEntries(
      [template.run, ...template.modules].map((node) => [node.id, node]),
    )
    const context: GeometryContext = {
      resolve: (id) => nodes[id] as never,
      children: template.modules,
      siblings: [],
      parent: null,
    }
    const group = template.snapshot
      ? buildCabinetGeometry(template.run, context)
      : buildCabinetGeometry(previewNode)
    if (template.snapshot) {
      for (const module of template.modules) {
        const child = buildCabinetGeometry(module, {
          ...context,
          children: [],
          parent: template.run,
        })
        child.position.set(...module.position)
        child.rotation.y = module.rotation
        group.add(child)
      }
    }
  ...
  const publishFloorplanPreview = useCallback(
    (next: CabinetPlacement, island = islandModeRef.current) => {
      ...
      let floorplanNode: AnyNode = template.snapshot
        ? { ...template.run, position: previewPosition, rotation: next.yaw }
        : stretch
          ? { ...node, width: stretch.length }
          : node
      let floorplanContextNodes: AnyNode[] = template.snapshot ? template.modules : []
      ...
      const sizeDimensions =
        !stretch && !island && !template.snapshot
          ? buildCabinetPlacementSizeDimensions({
              depth: livePreviewNode.depth,
  ...
  useEffect(() => {
    ...
    const withPlacementValidity = (
      ...
      const livePreviewNode = template.snapshot ? template.run : previewNodeRef.current
      ...
      const footprints = floorPlaced
        ? getFloorPlacedFootprints(floorPlaced, effectiveNode, {
            nodes: template.snapshot
              ? {
                  ...useScene.getState().nodes,
                  ...Object.fromEntries(template.modules.map((node) => [node.id, node])),
                }
              : useScene.getState().nodes,
          }).filter(
            (
    ...
    const resolveWallHitPlacement = (hit: WallHit): CabinetPlacement | null => {
      ...
      const insertion = template.snapshot
        ? null
        : resolveCabinetRunInsertion({
            hit,
    ...
    const buildRunNodes = (position: [number, number, number], yaw: number) => {
      const patch = previewNodeRef.current
    ...
    const onClick = (event: FloorPlacementClickTriggerEvent) => {
      ...
      if (!template.snapshot && useEditor.getState().getContinuation('cabinet') === 'continuous') {
        draftSegmentsRef.current = []
        setDraftSegments([])
        chainRootRunRef.current = null
        chainRunRef.current = null
      ...
      const assembly = template.snapshot
        ? instantiateCabinetPlacement(template, next.position, next.yaw, activeLevelId)
        : { run: cabinet, modules: [buildModule(0, previewNodeRef.current.width, 0)] }
      const nodes = {
        ...useScene.getState().nodes,
        [assembly.run.id]: assembly.run,
        ...Object.fromEntries(assembly.modules.map((node) => [node.id, node])),
      }
      const committedCabinet = CabinetNode.parse({
        ...assembly.run,
        ...resolveSupportSlabPatch(assembly.run, nodes),
      })
      useScene
        .getState()
        .createNodes([
          { node: committedCabinet, parentId: activeLevelId },
          ...assembly.modules.map((node) => ({ node, parentId: committedCabinet.id })),
        ])
      bumpCabinetRunsNearNewRun(committedCabinet.id as AnyNodeId)
      useViewer.getState().setSelection({ selectedIds: [assembly.modules[0]!.id] })
      useEditor.getState().setMode('select')
      triggerSFX('sfx:item-place')
      useAlignmentGuides.getState().clear()
      usePlacementPreview.getState().clear()
  ...
  }, [activeLevelId, metricNotation, publishFloorplanPreview, template, unit])
  ...
  const placementLabel = placement.insertionFailure
    ...
    : stretch
      ? placement.valid
        ? `${draftSegments.length + 1} 段柜列 · ${stretch.modules.length} 个模块 · 单击继续 · 双击或 Esc 完成`
        : null
      : !placement.valid
        ? null
  ...
  return (
    <LevelOffsetGroup>
      ...
      <PlacementBox
        activeDimensionId={stretch || template.snapshot ? null : activeDimensionId}
        dimensions={placementBoxDimensions}
        dimensionInput={dimensionInput}
        measurements={{ unit, metricNotation }}
        measurementValues={
          stretch || template.snapshot
            ? undefined
            : [previewNode.width, previewNode.carcassHeight, previewNode.depth]
        }
        onDimensionSelect={
          stretch || template.snapshot
            ? undefined
            : (id) => usePlacementPreview.getState().selectDimension(id)
        }
...
const CabinetTool = () => {
  const defaults = useEditor((state) => state.toolDefaults.cabinet)
  return <CabinetPlacementTool key={JSON.stringify(defaults)} defaults={defaults} />
}
