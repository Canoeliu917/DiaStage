...
const WindowTool: React.FC = () => {
  const toolDefaults = useEditor((state) => state.toolDefaults.window)
  const defaults = useMemo(() => windowToolParameters(toolDefaults), [toolDefaults])
  const presetMaterials = toolDefaults?.sceneMaterials
  ...
  const ghostStub = useMemo(
    ...
    [defaults, fallbackPose?.side],
  )
  // Facing-pose events read the current preset depth without depending on side flips.
  const frameDepthRef = useRef(ghostStub.frameDepth)
  frameDepthRef.current = ghostStub.frameDepth
  ...
  useEffect(() => {
    ...
    const applyWallTarget = (args: {
      ...
      const width = draftRef.current?.width ?? defaults.width
      const height = draftRef.current?.height ?? defaults.height

      if (!draftRef.current) {
        const node = WindowNode.parse({
          ...defaults,
          position: [0, DEFAULT_WINDOW_SILL_M + defaults.height / 2, 0],
          rotation: [0, itemRotation, 0],
          side,
          wallId: wall.id,
          parentId: wall.id,
    ...
    const commitWindowAtWall = (
      ...
      createNodesWithSceneMaterials([{ node, parentId: wall.id as AnyNodeId }], presetMaterials)
      selectNode(node.id)
  ...
  }, [activeLevelId, defaults, isCameraDragging, presetMaterials, selectNode])

  // Cursor geometry changes only when the selected preset dimensions change.
  const edgesGeo = useMemo(() => {
    const boxGeo = new BoxGeometry(defaults.width, defaults.height, 0.07)
