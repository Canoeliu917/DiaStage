...
const COLUMN_PRESET_LABELS: Record<ColumnPresetId, string> = {
  ...
  boxFrameSupport: '箱形框架',
}

const COLUMN_PRESET_OPTIONS = Object.entries(COLUMN_PRESETS).map(([value]) => ({
  value: value as ColumnPresetId,
  label: COLUMN_PRESET_LABELS[value as ColumnPresetId],
}))
...
const SUPPORT_STYLE_OPTIONS: Array<{ label: string; value: ColumnNode['supportStyle'] }> = [
  ...
  { label: 'A 形支架', value: 'a-frame' },
  { label: 'Y 形支撑', value: 'y-frame' },
  { label: 'V 形支撑', value: 'v-frame' },
  { label: 'X 形支撑', value: 'x-brace' },
  { label: 'K 形支撑', value: 'k-brace' },
  { label: '单斜撑', value: 'single-strut' },
  { label: '三脚架', value: 'tripod' },
  { label: '支架', value: 'trestle' },
  { label: '门式框架', value: 'portal-frame' },
  { label: '箱形框架', value: 'box-frame' },
]
    ...
    >
      ...
      <PanelSection title="尺寸">
        {managedByLeanTo && (
          <p className="px-1 pt-1 text-muted-foreground text-xs leading-relaxed">
            高度和平面轮廓由附属棚顶控制。可在此旋转或更改支撑样式；尺寸请在所属棚顶中调整。
          </p>
        )}
        ...
        {isBraceSupport ? (
          <>
            ...
            <SliderControl
              label={supportStyle === 'y-frame' ? '分叉展开宽度' : '顶部展开宽度'}
              max={
                supportStyle === 'y-frame' ||
                supportStyle === 'v-frame' ||
                supportStyle === 'x-brace' ||
      ...
      {!isBraceSupport && (
        <PanelSection title="柱身">
          ...
          >
            ...
            <option value="tapered">渐缩</option>
            <option value="bulged">鼓形</option>
      ...
      {!isBraceSupport && (
        <PanelSection title="端部">
          ...
          >
            <option value="none">无柱头</option>
            <option value="simple">简洁柱头</option>
            <option value="stepped">阶梯形柱头</option>
            <option value="rounded">圆形柱头</option>
          </select>
          ...
          >
            <option value="none">无柱础</option>
            <option value="simple-square">简洁块状柱础</option>
            <option value="square-plinth">方形台座柱础</option>
            <option value="stepped-square">阶梯形柱础</option>
            <option value="round-rings">圆形柱础</option>
          </select>
          {node.baseStyle !== 'none' && (
            <SliderControl
              label="底部高度"
