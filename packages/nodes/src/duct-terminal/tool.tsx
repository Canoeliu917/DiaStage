    ...
    <LevelOffsetGroup>
      ...
      >
        <div className="flex items-center gap-2 whitespace-nowrap rounded-full border border-border/60 bg-background/90 px-4 py-1.5 text-xs tabular-nums shadow-sm backdrop-blur">
          {placement.snapped && (
            <>
              <span className="font-medium text-primary">已吸附到风管</span>
              <span aria-hidden className="text-muted-foreground">
                ·
              </span>
            </>
          )}
          <span className="font-medium text-foreground">安装位置 {mountLabel}</span>
          <span aria-hidden className="text-muted-foreground">
            ·
          </span>
          <span className="text-muted-foreground">M 切换安装面</span>
          {effectiveMount !== 'wall' && (
            <>
              <span aria-hidden className="text-muted-foreground">
                ·
