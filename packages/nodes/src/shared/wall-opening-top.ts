 ...
 */
export type WallTopSceneReader = {
 ...
 * is the wall top an opening must stay under.
 *
 ...
 */
export function resolveWallOpeningTop(
  wall: WallNode,
  nodes: Readonly<Record<AnyNodeId, AnyNode>>,
): number {
 ...
 * the opening is unhosted (no wallId, or the wall is gone).
 */
export function readHostWallTop(
  wallId: string | null | undefined,
  scene: WallTopSceneReader,
): number {
  ...
  return resolveWallOpeningTop(wall, scene.nodes())
}
