...
export function buildDoorFloorplanSchedule(args: {
  ...
  return {
    ...
    title: '门明细表',
    columns: [
      { key: 'mark', label: '编号', weight: 0.65 },
      { key: 'type', label: '类型', weight: 1.25 },
      { key: 'size', label: '名义尺寸', weight: 1.35 },
      { key: 'roughOpening', label: '预留洞口', weight: 1.35 },
      { key: 'operation', label: '开启方式', weight: 1.35 },
      { key: 'frame', label: '框厚／框深', weight: 1.25 },
      { key: 'hardware', label: '五金件', weight: 1.35 },
    ],
    rows: args.siblings.map((door) => ({
      id: door.id,
      cells: {
        mark: marks.markById.get(door.id) ?? '—',
        type: door.openingKind === 'opening' ? '洞口' : titleCase(door.doorType),
...
export function buildWindowFloorplanSchedule(args: {
  ...
  return {
    ...
    columns: [
      { key: 'mark', label: '编号', weight: 0.65 },
      { key: 'type', label: '类型', weight: 1.2 },
      { key: 'size', label: '名义尺寸', weight: 1.35 },
      { key: 'roughOpening', label: '预留洞口', weight: 1.35 },
      { key: 'sill', label: '窗台', weight: 0.9 },
      { key: 'head', label: '窗顶', weight: 0.9 },
      { key: 'operation', label: '开启方式', weight: 1.35 },
    ],
    rows: args.siblings.map((window) => ({
      ...
      cells: {
        ...
        type: window.openingKind === 'opening' ? '洞口' : titleCase(window.windowType),
...
function resolveOpeningMarks<T extends OpeningNode>(
  ...
  const issues = [...explicitMarks.entries()]
    ...
    .map(
      ([mark, ids]) => `${kind === 'door' ? '门' : '窗'}编号 ${mark} 重复（${ids.length} 个实例）`,
    )
...
function doorOperation(door: DoorNode): string {
  if (door.openingKind === 'opening') return '无'
  ...
  if (door.doorType === 'sliding' || door.doorType === 'pocket' || door.doorType === 'barn') {
    return `向${titleCase(door.slideDirection)}推拉`
  }
...
function doorHardware(door: DoorNode): string {
  if (door.openingKind === 'opening') return '无'
  const hardware = []
  if (door.doorCloser) hardware.push('闭门器')
  if (door.panicBar) hardware.push('逃生推杠')
  if (door.threshold) hardware.push('门槛')
  return hardware.length > 0 ? hardware.join('、') : '标准'
}

function windowOperation(window: WindowNode): string {
  if (window.openingKind === 'opening') return '无'
  if (window.windowType === 'fixed') return '固定'
  if (window.windowType === 'casement') {
    return window.casementStyle === 'french' ? '双扇平开' : `${titleCase(window.hingesSide)}侧铰链`
  }
