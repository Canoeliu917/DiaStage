export function shouldHandleWalkthroughLook(event: MouseEvent, canvas: HTMLCanvasElement) {
  return (
    canvas.ownerDocument.pointerLockElement === canvas ||
    (event.target === canvas && (event.buttons & 1) !== 0)
  )
}
 ...
 */
export function requestWalkthroughPointerLock(options?: {
  canvas?: HTMLCanvasElement
  retryWhile?: () => boolean
}) {
  const canvas =
    options?.canvas ?? document.querySelector<HTMLCanvasElement>('[data-pascal-viewer-3d] canvas')
  if (!canvas) return
  ...
  try {
    ...
    if (result && typeof result.catch === 'function') {
      result.catch(() => {
        ...
        window.setTimeout(() => {
          if (!canvas.isConnected || !retryWhile()) return
          if (document.pointerLockElement === canvas) return
          try {
