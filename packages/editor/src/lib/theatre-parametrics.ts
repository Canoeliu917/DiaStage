import type { AnyNode, DoorNode, ParametricDescriptor, ParamField, StairNode } from '@pascal-app/core'
...
const SCENIC_FIELDS: Record<string, string[]> = {
  ...
  window: ['width', 'height'],
  fence: ['style', 'height', 'thickness', 'length', 'curve', 'postSpacing', 'postSize', 'groundClearance'],
...
const STAIR_GROUPS = [
  { label: '台阶尺寸', fields: [{ key: 'width', kind: 'number', unit: 'm', min: 0.1, max: 100, step: 0.05 }] },
  { label: '落位', fields: [{ key: 'position', kind: 'vec3' }] },
] satisfies ParametricDescriptor<StairNode>['groups']

const OPENING_GROUPS = [{
  label: '开合',
  fields: [{ key: 'operationState', label: '开合程度', kind: 'number', min: 0, max: 1, step: 0.05 }],
...
export function theatreParametrics(type: string | null, source: ParametricDescriptor<AnyNode> | undefined): TheatreParametricDescriptor | undefined {
  if (!type || !source || !isTheatreEditableType(type)) return undefined
  if (type === 'block' || type === 'item') return { ...source, actions: undefined, trailingSection: undefined }
  if (type === 'stair' || type === 'stair-segment') return {
    ...source,
  ...
  const groups: TheatreParametricDescriptor['groups'] = source.groups.map((group) => ({ ...group, fields: group.fields.filter((field) => keys.includes(String(field.key))) })).filter((group) => group.fields.length > 0)
