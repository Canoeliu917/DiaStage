...
/** Commit a preset's nodes and embedded materials together without touching the clipboard. */
export function createNodesWithSceneMaterials(
  ops: { node: AnyNode; parentId?: AnyNodeId }[],
  materials?: unknown,
): void {
  const supplied = z.record(z.string(), SceneMaterial).parse(materials ?? {})
  const scene = useScene.getState()
  const materialIdMap = new Map<SceneMaterialId, SceneMaterialId>()
  const materialsToCreate: SceneMaterial[] = []
  for (const material of Object.values(supplied)) {
  ...
  const mapped = ops.map((op) => ({
    ...op,
    node: remapSceneMaterialReferences(op.node, materialIdMap) as AnyNode,
  }))
  runAsSingleSceneHistoryStep(useScene, () => {
    for (const material of materialsToCreate) scene.addSceneMaterial(material)
    scene.createNodes(mapped)
  })
}
