...
const EDITABLE_TYPES = new Set(['block', 'wall', 'fence', 'shelf', 'door', 'window', 'stair', 'stair-segment', 'item', 'column', 'slab', 'zone', 'spawn'])
...
const LABELS: Record<string, string> = {
  site: '场地', building: '舞台空间', level: '表演层', zone: '表演区',
  block: '台块', wall: '景片', fence: '栏杆', shelf: '置物架',
  door: '实用门', window: '实用窗', stair: '台阶', 'stair-segment': '台阶段',
  slab: '舞台地面', ceiling: '上空遮挡', column: '立柱', item: '道具与陈设',
  scan: '场地参考', guide: '平面参考', spawn: '观察起点',
}
...
export function getTheatreNodeName(node: AnyNode): string {
  ...
  if (name && !/^(Site|Building|Level|Wall|Roof|Slab|Ceiling|Zone|Stair|建筑|楼层|墙体|屋顶|楼板|天花板)(\s*[\d.-]+)?$/i.test(name)) return name
