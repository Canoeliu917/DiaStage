...
export interface EditorProps {
  ...
  /** Desktop sidebar content above the tab rail and active tool panel. */
  ...
  /** Host runtime mounted in both editing and Preview React Three Fiber scenes. */
  viewerRuntimeSlot?: ReactNode
  /** Host-owned scene content mounted only in the Studio workspace's active 3D surface. */
  studioSceneSlot?: ReactNode
  ...
  return (
    <div className="fixed inset-0 z-80 flex items-center justify-center bg-background/95 p-4 text-foreground">
      <div className="w-full max-w-md rounded-2xl border border-border/60 bg-background p-6 shadow-xl">
        ...
        <p className="mt-2 text-muted-foreground text-sm">可以重试加载场景或返回首页。</p>
        ...
        >
          ...
          >
            <ViewerSceneContent
              ...
              viewerRuntimeSlot={viewerRuntimeSlot}
              studioSceneSlot={show3d && !isPreviewMode ? studioSceneSlot : undefined}
            />
          </Viewer>
        </div>
      </div>
...
function EditorContent({
  ...
  const visibleSidebarTabs = sidebarTabs?.filter(
    (tab) => !tab.workspaces || tab.workspaces.includes(isStudioMode ? 'studio' : 'edit'),
  )
  ...
  // into its tab bar.
  const registeredHostRailPanels = useHostPanels()
  const hostRailPanels = showPluginPanels ? registeredHostRailPanels : []
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setSceneLoadError(error ?? new Error('场景加载失败'))
        }
      } finally {
        if (!cancelled) {
  ...
  const previewViewerContent = (
    ...
    >
      ...
      <ThumbnailGenerator onThumbnailCapture={onThumbnailCapture} />
      <InteractiveSystem />
      {previewStageMode !== '2d' && viewerRuntimeSlot}
    </Viewer>
  )
  ...
  const viewerCanvas = (
    <ViewerCanvas
      ...
      viewerSceneSlot={viewerSceneSlot}
      viewerRuntimeSlot={viewerRuntimeSlot}
      studioSceneSlot={studioSceneSlot}
  ...
  if (layoutVersion === 'v2') {
    ...
    const tabMap = new Map<string, SidebarTab & { component: React.ComponentType }>(
      visibleSidebarTabs?.map((t) => [t.id, t]) ?? [],
    )
