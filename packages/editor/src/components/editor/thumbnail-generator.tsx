      ...
      await runSnapshotCapture(
        ...
        async () => {
          ...
          if (pipeline) {
            ...
            if (!result) throw new Error('快照拍摄未生成图像')
            blob = result.blob
            ...
            if (captureMode === 'viewport') {
              ...
              blob = await new Promise<Blob>((resolve, reject) =>
                offscreen.toBlob(
                  (b) => (b ? resolve(b) : reject(new Error('画布截图失败'))),
                  SNAPSHOT_MIME,
                  SNAPSHOT_QUALITY,
                ),
              )
          ...
          if (
            ...
            throw new Error('截图期间场景已变更，请重试。')
          }
          if (cameraPose && event.projectId !== useViewer.getState().projectId) {
            throw new Error('截图期间当前项目已切换')
          }
