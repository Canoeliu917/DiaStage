      ...
      <div className="flex flex-col gap-0.5">
        ...
        {showScaleHint && (
          <div className="flex items-center gap-1.5 opacity-40">
            <span className="font-medium text-[11px] lowercase leading-none">设置比例</span>
            <Ruler aria-hidden="true" className="h-3.5 w-3.5 shrink-0" strokeWidth={2.2} />
            <span className="font-medium text-[11px] lowercase leading-none">面板</span>
          </div>
        )}
      </div>
    </div>
...
export function FloorplanPanel({
  ...
  const doorToolDefaults = useEditor((state) => state.toolDefaults.door)
  const windowToolDefaults = useEditor((state) => state.toolDefaults.window)
  const openingDefaults = useMemo(
    () => ({
      door: DoorNodeSchema.parse(doorToolDefaults ?? {}),
      window: WindowNodeSchema.parse(windowToolDefaults ?? {}),
    }),
    [doorToolDefaults, windowToolDefaults],
  )
  ...
  const floorplanOpeningLocalY = useMemo(() => {
    ...
    if (isWindowBuildActive) {
      ...
      return snapToHalf(openingDefaults.window.height / 2 + 0.5)
    }

    return isDoorBuildActive ? openingDefaults.door.height / 2 : 0
  }, [isDoorBuildActive, isWindowBuildActive, movingNode, openingDefaults])
  ...
  const showOpeningGhost = useCallback(
    ...
    [floorplanOpeningLocalY, isDoorBuildActive, movingNode, movingOpeningType, openingDefaults],
  )
          ...
          >
            <div className="mb-3 flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                <div className="font-medium text-sm">设置叠加图比例</div>
                <div className="mt-0.5 text-muted-foreground text-xs leading-4">
                  输入刚绘制线段的实际长度，图像会缩放至对应比例。
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 rounded-lg bg-muted/45 px-3 py-2 text-muted-foreground text-xs">
              {pendingReferenceImageScaleFactor
                ? `图像将以第一个点为基准缩放 ${formatNumber(pendingReferenceImageScaleFactor, 3)} 倍。`
                : '请输入大于 0 的长度。'}
            </div>
