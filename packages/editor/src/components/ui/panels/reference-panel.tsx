  ...
  const handleReplaceFile = useCallback(
    async (file: File) => {
      ...
      if (!file.type.startsWith('image/')) {
        setReplaceError('请选择 PNG、JPEG 或 WebP 图像。')
        return
      }
          ...
          <PanelSection title="图像">
            ...
            <ActionGroup>
              <ActionButton
                ...
                label={node.visible === false ? '显示' : '隐藏'}
          ...
          <PanelSection title="参考比例">
            ...
            {!node.scaleReference && (
              <p className="px-0.5 text-muted-foreground text-xs leading-snug">
                {isScaleFlowActive
                  ? '点击图纸上已知距离的两端，再输入实际长度。'
                  : '沿图纸上的已知尺寸画线，再输入实际长度以精确缩放图像。'}
              </p>
            )}
            ...
            <ActionGroup>
              <ActionButton
                ...
                label={isScaleFlowActive ? '取消' : node.scaleReference ? '编辑比例' : '设置比例'}
            ...
            {node.scaleReference && (
              <ActionGroup>
                <ActionButton
                  label={scaleReferenceVisible ? '隐藏比例' : '显示比例'}
