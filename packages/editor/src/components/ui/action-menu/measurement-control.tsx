...
import type { ConstructionDimensionChainMode, ConstructionDimensionMode } from '@pascal-app/core'
import {
  Box,
  Check,
  ChevronDown,
...
const measurementOptions = [
  { kind: 'distance', label: '距离', icon: Ruler },
  { kind: 'angle', label: '角度', icon: Triangle },
  { kind: 'area', label: '面积', icon: Square },
  { kind: 'perimeter', label: '周长', icon: Waypoints },
  { kind: 'volume', label: '体积', icon: Box },
...
const measurementMenuOptions = [
  { kind: 'smart', label: '智能', icon: ScanSearch },
  ...measurementOptions,
] as const

const constructionDimensionOptions = [
  { mode: 'linear', chainMode: 'point-to-point', label: '线性尺寸', icon: Ruler },
  { mode: 'linear', chainMode: 'continuous', label: '连续尺寸', icon: Waypoints },
  { mode: 'radius', chainMode: 'point-to-point', label: '半径尺寸', icon: CircleIcon },
  { mode: 'diameter', chainMode: 'point-to-point', label: '直径尺寸', icon: CircleIcon },
  { mode: 'center-mark', chainMode: 'point-to-point', label: '圆心标记', icon: Crosshair },
  { mode: 'chord', chainMode: 'point-to-point', label: '弦长尺寸', icon: Minus },
  { mode: 'arc-length', chainMode: 'point-to-point', label: '弧长', icon: CircleIcon },
  { mode: 'angular', chainMode: 'point-to-point', label: '角度尺寸', icon: Triangle },
  { mode: 'coordinate', chainMode: 'continuous', label: '坐标尺寸', icon: Grid2X2 },
] as const satisfies readonly {
...
export function MeasurementControl() {
  ...
  const selectedLabel = isConstructionDimensionActive
    ? (activeConstructionDimensionOption?.label ?? '线性尺寸')
    : isSmartActive
      ? '智能'
  ...
  const activateConstructionDimension = (
    ...
    if (useFloorplanMode.getState().mode !== 'expert') {
      useFloorplanMode.getState().showExpertModeNotice('施工尺寸')
      return
    }
    setPhase('structure')
    setStructureLayer('elements')
  ...
  return (
    <Popover onOpenChange={setIsOpen} open={isOpen}>
        ...
        <PopoverTrigger asChild>
          <button
            ...
            className={cn(
              ...
              isOpen ? 'bg-cyan-500/15 text-cyan-400' : 'hover:bg-cyan-500/10 hover:text-cyan-400',
            )}
            type="button"
          >
            <ChevronDown
      ...
      <PopoverContent
        ...
        sideOffset={14}
      >
        <div aria-label="测量类型" className="space-y-1" role="menu">
