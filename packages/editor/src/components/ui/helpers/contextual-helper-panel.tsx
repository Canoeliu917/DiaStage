    ...
    <div className={KEY_CELL_CLASS}>
      {keys.map((entry, index) => (
        <Fragment key={`${String(entry)}-${index}`}>
          {index > 0 ? (
            <span className="font-medium text-[12px] text-muted-foreground/80 leading-none">+</span>
          ) : null}
          {Array.isArray(entry) ? (
            entry.map((alternative, altIndex) => (
              <Fragment key={`${alternative}-${altIndex}`}>
                ...
                <ShortcutToken
                  className={cn(
                    TOKEN_CLASS,
                    active && 'border-white bg-white text-black shadow-sm',
                  )}
                  value={alternative}
                />
              </Fragment>
...
function ChipRow({
  ...
  if (!onClick) {
    return (
      <div className={cn(ROW_CLASS, 'items-center', disabled && 'opacity-45 saturate-0')}>
        {body}
      </div>
    )
  }
...
function SnappingChips({ context }: { context: SnapContext }) {
  ...
  return (
    <>
      <ChipRow
        ariaLabel={`吸附：${SNAPPING_MODE_LABELS[snappingMode]}`}
...
function ContinuationChip({ context }: { context: ContinuationContext }) {
  ...
  return (
    <ChipRow
      ariaLabel={`连续绘制：${label}`}
      icon={icon}
      label={label}
      onClick={() => cycleContinuation(context)}
      shortcut="C"
      tooltip="连续绘制：点击或按 C 切换"
    />
  )
}
  ...
  return (
    <>
      ...
      <ChipRow
        ...
        shortcut="C"
        tooltip={isCurved ? '曲线围栏不支持直线连续绘制' : '直线围栏连续绘制：点击或按 C 切换'}
      />
      ...
      {isCurved && curveStarted ? (
        <ChipRow icon="lucide:circle-check" label="完成曲线（或双击）" shortcut="Enter" />
      ) : null}
    </>
  )
}
...
function PaintScopeChip() {
  ...
  if (!(paintEraser || hasActivePaintMaterial(activePaintMaterial))) {
    return <ChipRow icon="lucide:palette" label="选择要绘制的材质" />
  }

  // Not over anything paintable → guide the user to hover, still teaching Shift.
  if (!paintHover) {
    return (
      <ChipRow icon="lucide:mouse-pointer-click" label="将鼠标移至表面以绘制" shortcut="Shift" />
    )
  }
  ...
  if (scopes.length <= 1) {
    return (
      <ChipRow
        ...
        label={`绘制：${paintScopeLabel(effective, paintHover)}`}
      />
    )
  }
  ...
  return (
    <ChipRow
      ariaLabel={`绘制范围：${paintScopeLabel(effective, paintHover)}`}
      icon={PAINT_SCOPE_ICONS[effective]}
      label={`绘制：${paintScopeLabel(effective, paintHover)}`}
      onClick={() => cyclePaintScope()}
      shortcut="Shift"
      tooltip="绘制范围：点击或按 Shift 切换"
    />
  )
}
...
export function ContextualHelperPanel({
  ...
  return (
    <div className={CONTAINER_CLASS} data-contextual-helper>
      ...
      {hints.map((hint) => (
        <div className={cn(ROW_CLASS, 'items-start')} key={`${hint.keys.join('+')}:${hint.label}`}>
