...
function filterBySource(items: MaterialCatalogItem[], filter: MaterialSourceFilter) {
  return items.filter((item) => (item.source ?? 'builtin') === filter)
}
...
const BUILTIN_MATERIAL_LABELS: Record<string, string> = {
  'wood-finewood27': '精细木纹',
  'wood-floorplank1': '舞台木板',
  'preset-white': '白色',
...
export function MaterialPicker({
  ...
  const [sourceFilter, setSourceFilter] = useState<MaterialSourceFilter>('builtin')
  ...
  const visibleSourceFilters = SOURCE_FILTERS.filter(
    (filter) => filter.id === 'builtin' || dynamicSources.has(filter.id) ||
  ...
  useEffect(() => {
    ...
    if (entry?.category) {
      ...
      setSourceFilter(entry.source ?? 'builtin')
    }
  }, [selectedMaterialPreset])
