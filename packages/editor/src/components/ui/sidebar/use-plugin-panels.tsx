...
function PluginPanelCrashed({ label }: { label: string }) {
  return (
    <div className="flex flex-col gap-2 p-4 text-sm">
      <p className="font-medium text-sidebar-foreground">"{label}”插件发生错误</p>
      <p className="text-sidebar-foreground/50 text-xs">
        此面板发生错误，已在当前会话中卸载。编辑器其他部分可继续使用，重新加载可重试。
      </p>
    </div>
  )
}
...
function resolvePanelComponent(panel: EditorHostPanel): ComponentType {
  ...
  const Wrapped: ComponentType = () => (
    <ErrorBoundary fallback={<PluginPanelCrashed label={panel.label} />}>
      <Suspense fallback={<div className="p-4 text-sidebar-foreground/50 text-sm">正在加载…</div>}>
        <Lazy />
      </Suspense>
    </ErrorBoundary>
  )
