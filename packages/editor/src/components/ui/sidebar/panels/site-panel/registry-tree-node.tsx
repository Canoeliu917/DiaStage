...
export const RegistryTreeNode = memo(function RegistryTreeNode({
  ...
  const hostChildren = node ? editorHostTreeChildrenRegistry.childrenForKind(node.type) : undefined
  ...
  const iconElement =
    icon?.kind === 'iconify' ? (
      ...
      <Image alt="" className="object-contain opacity-60" height={14} src={iconSrc} width={14} />
    )
  const snapTarget = resolveNodeSnapTarget(node)
  const defaultName = node
    ? tree?.label?.(node, useScene.getState().nodes) || node.name || presentation?.label || '节点'
    : '节点'
