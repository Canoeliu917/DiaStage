...
import { prettifyEnumValue } from '../../../panels/parametric-field-utils'
...
export const RoofTreeNode = memo(function RoofTreeNode({
  ...
  const defaultName = `屋顶（${segmentCount} 个分段）`
...
function RoofSegmentTreeNode({
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      const label = `${prettifyEnumValue(node.roofType)}（${node.width.toFixed(1)}×${node.depth.toFixed(1)}m）`
  ...
  const defaultName = `${prettifyEnumValue(node.roofType)}（${node.width.toFixed(1)}×${node.depth.toFixed(1)}m）`
