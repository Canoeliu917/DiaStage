...
function groupSchemaIssuesByType(issues: SchemaIssue[]): { type: string; issues: SchemaIssue[] }[] {
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {fileName} · {formatFileSize(fileSizeBytes)} · {stats.total} 个节点
          </DialogDescription>
        </DialogHeader>
                ...
                <div>
                  ...
                  {stats.floorAreaM2 > 0 && (
                    <div className="flex items-center justify-between border-t px-3 py-2">
                      <span className="text-muted-foreground text-sm">楼面面积</span>
            ...
            <div className="space-y-2 rounded-md border bg-card p-3">
              ...
              >
                <span className="font-medium text-muted-foreground text-xs uppercase">
                  结构详情（{schemaIssueCount} 个节点 ）
                </span>
                <span className="text-muted-foreground text-xs">
                  {showSchemaIssues ? '隐藏' : '显示'}
                </span>
              </button>
              {showSchemaIssues && (
