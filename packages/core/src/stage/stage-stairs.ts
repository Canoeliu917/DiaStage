...
export function stageStairBounds(stair: StairNode, nodes: Readonly<Record<string, AnyNode>>) {
  ...
  if (!box) throw new Error('台阶缺少可用的踏步几何。')
  ...
  return {
    dimensions: [box.maxX - box.minX, height, box.maxZ - box.minZ] as [number, number, number],
    boundsCenter: [(box.minX + box.maxX) / 2, height / 2, (box.minZ + box.maxZ) / 2] as [
      number,
      number,
      number,
    ],
...
export function readStageStair(
  ...
  if (
    ...
    stair.children.length !== 1 ||
    segment?.type !== 'stair-segment' ||
    segment.segmentType !== 'stair'
  )
    throw new Error(
      '该台阶包含多个组成部分，可整体移动、复制和复台；调整踏步尺寸请添加新的舞台台阶。',
    )
