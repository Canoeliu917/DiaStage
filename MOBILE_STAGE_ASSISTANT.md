...
日期：2026-09-10。实施基线：`6419323b02644c5e9608f20c56eead0ed585462c`。
...
1. 电脑打开已保存剧目，进入「置景 → 舞台口令 → 连接手机舞台助手」，生成配对码。
...
本地验证入口：`http://127.0.0.1:4323/`、`/remote-voice`。回环地址仅能在该电脑访问；不是已配置好的手机 HTTPS 服务。
...
- **未配置/未实测**：手机可访问的 HTTPS 服务。遵守不部署要求；`NEXT_PUBLIC_DIASTAGE_REMOTE_URL` 只能填访问地址，不能填密钥。公网拥有者身份和授权连续制景继续禁用，不能称为公网生产就绪。
